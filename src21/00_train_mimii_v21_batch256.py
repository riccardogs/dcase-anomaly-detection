"""
@file   00_train_mimii_v21_batch256.py
@brief  Training with Hybrid V2 + Augmentation (batch 256)
"""
...
print("=" * 60)
print("MODEL 21 BATCH256 - HYBRID V2 (WITH AUGMENTATION)")
print(f"GPU: {tf.config.list_physical_devices('GPU')}")
print("=" * 60)

# Usa il nuovo YAML con batch 256
param = com.yaml_load()
# Sovrascrivi con il file batch256 se esiste
if os.path.exists("baseline_mimii_v21_batch256.yaml"):
    import yaml
    with open("baseline_mimii_v21_batch256.yaml") as f:
        param_batch = yaml.safe_load(f)
        param["fit"]["batch_size"] = param_batch["fit"]["batch_size"]

print(f"📌 Batch size: {param['fit']['batch_size']}")
...
if __name__ == "__main__":
    ...
    for idx, target_dir in enumerate(machine_dirs):
        ...
        # Crea generator con batch size ridotto
        train_gen = DataGenerator(train_data_small, batch_size=batch_size)
        val_gen = DataGenerator(val_data_small, batch_size=batch_size, shuffle=False)
