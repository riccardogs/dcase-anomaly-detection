"""
@file   00_train_mimii_v21.py
@brief  Training with Hybrid V2 + Augmentation (IDENTICO src20 + augmentation)
"""
...
print("=" * 60)
print("MODEL 21 - HYBRID V2 TRAINING (WITH AUGMENTATION)")
...
def augment_data(data, strength=0.01, mixup_alpha=0.5, augment_ratio=0.25):
    """Data augmentation with noise and mixup"""
...
def list_to_vector_array(file_list, msg="calc...", n_mels=128, frames=5, 
                          ...
                          augment=True, strength=0.01, mixup_alpha=0.5, augment_ratio=0.25):
    """Convert file list to vector array WITH augmentation"""
    dims = n_mels * frames
    dataset = None
    ...
    if augment and dataset is not None and len(dataset) > 0:
        dataset = augment_data(dataset, strength, mixup_alpha, augment_ratio)
...
if __name__ == "__main__":
    ...
    print(f"   Noise strength: {aug_strength}")
    print(f"   Mixup alpha: {aug_mixup}")
    print(f"   Augment ratio: {aug_ratio}")
